...
from rsl_rl.network import NetworkWrapper, network_cfg
from rsl_rl.network.util import get_activation

@configclass
class RNDConfig:
    num_states: dict[str, list[int]] = MISSING
    num_outputs: int = MISSING
    predictor_cfg: network_cfg.NetworkConfig = MISSING
    target_cfg: network_cfg.NetworkConfig = MISSING
    weight: float = 0.0
    ...
    def __post_init__(self):
        self.predictor_cfg.state_aggr_block_cfg.network_cfg.output_dim = [self.num_outputs]
        self.target_cfg.state_aggr_block_cfg.network_cfg.output_dim = [self.num_outputs]
...
class RandomNetworkDistillation(nn.Module):
    ...
    def __init__(
        ...
        if cfg is None:   
            ...
            self.cfg = cfg
            self.predictor = NetworkWrapper(self.cfg.predictor_cfg,
                                            self.cfg.num_states)
            self.target = NetworkWrapper(self.cfg.target_cfg,
                                         self.cfg.num_states)
            
            self.initial_weight = self.cfg.weight
            state_normalization = self.cfg.state_normalization
            reward_normalization = self.cfg.reward_normalization

            weight_schedule = self.cfg.weight_schedule
        

        # Normalization of input gates
        if state_normalization:
            self.state_normalizer = EmpiricalNormalization(shape=[self.num_states], 
                                                           until=1.0e8).to(self.device)
        else:
            state_normalizer = torch.nn.Identity()
    ...
    def get_loss(self, rnd_state):
        # Necessary?
        # self.state_normalizer.eval()
        rnd_state = self.state_normalizer(rnd_state)
        pred_emb = self.predictor(rnd_state)
        target_emb = self.target(rnd_state).detach()
